...
The official checkpoint is hosted on OneDrive / Google Drive. We try, in order:

1. If the file already exists at the target path, verify size and optional
   SHA256 and exit.
2. OneDrive shared-file direct download via the public API endpoint.
3. Google Drive via ``gdown`` if installed.
4. Fall back to manual-download instructions.

These are ImageNet classification weights for HRNet's backbone only. The pose
head is never loaded from this checkpoint.
"""
...
def _looks_like_real_ckpt(dest: Path) -> bool:
    ...
    if size < MIN_EXPECTED_BYTES:
        print(
            f"[download] file at {dest} is only {size} bytes "
            f"(<{MIN_EXPECTED_BYTES}); looks like an error page, discarding"
        )
...
def _print_manual_instructions(dest: Path) -> None:
    print(
        ...
        "Manual steps:\n"
        "  1. Open https://github.com/HRNet/HRNet-Image-Classification\n"
        "  2. Download the HRNet-W32-C checkpoint from OneDrive or Google Drive\n"
        "     (file id 1zYC7go9EV0XaSlSBjMaiyE_4TcHc_S38).\n"
